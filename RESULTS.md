...
**Environment:** LocalStack community edition **3.8.1** (emulated AWS, running in Docker): **NOT real AWS**.
**Data:** 100% **synthetic**, seeded, reproducible (see `src/deedstream/data_gen.py`). No real county records were scraped or used.

All numbers below come from runs against LocalStack-hosted S3, SQS and
DynamoDB. Raw machine-readable values are committed under `results/*.json`.
Anything not measured is marked as such.


## How the numbers were produced

Two complementary paths, both on LocalStack:
...
| **Cost / month** | not measured (LocalStack emits no billing data) | `results/cost.json` |
...
LocalStack is free and local and emits **no billing
...
- **CloudWatch alarms not built.** LocalStack community does not meaningfully
  ...
  and the simulated-failure recovery-time metric are **not measured**. The
